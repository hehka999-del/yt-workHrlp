...
-- Уничтожение старого GUI для предотвращения дублирования
...
-- ==========================================
-- ЗАГЛУШКИ ДЛЯ ОТСУТСТВУЮЩИХ ФУНКЦИЙ И ПЕРЕМЕННЫХ
-- (Они нужны, чтобы скрипт не крашился при активации функций)
-- ==========================================
local toxicPhrases = {
    "изи таргет!", "слабый челик, поплачь", "ты под моим контролем!", 
    "Legenly Hub на высоте!", "купи прямые руки", "отдыхай в лобби"
}
...
local function RunOrbitFling(target)
    ...
    Notify("Fling", string.format(GetText("NotifyFlingStart"), target.DisplayName), CurrentTheme.Accent)
    
    task.spawn(function()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        
        while TrollState.FlingActive and target and target.Character do
            local tHrp = target.Character:FindFirstChild("HumanoidRootPart")
            if tHrp and hrp then
                -- FE Fling Логика (высокая скорость вращения и налет на цель)
        ...
        TrollState.FlingActive = false
        Notify("Fling", GetText("NotifyFlingEnd"), CurrentTheme.Accent)
    end)
end
...
local function UseWeaponOnTarget(targetChar)
    pcall(function()
        ...
        if tool and targetChar then
            tool:Activate()
            local handle = tool:FindFirstChild("Handle")
            local tHrp = targetChar:FindFirstChild("HumanoidRootPart")
            if handle and tHrp then
...
local function ApplyEjaculationFX()
    -- Кастомный визуальный эффект (например, белые партиклы)
    pcall(function()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then
...
-- ==========================================
-- ИНИЦИАЛИЗАЦИЯ ИНТЕРФЕЙСА (ДЕТЕКТ ЭКСКУТОРА)
-- ==========================================
...
-- ==========================================
-- КОНСТРУКТОРЫ ЭЛЕМЕНТОВ ИНТЕРФЕЙСА (ВЕРНЫЙ ПОРЯДОК)
-- ==========================================
local Sidebar, Pages, MainPage, InfoPage, DropdownFrame
...
-- ==========================================
-- ПОЛНОЦЕННАЯ СБОРКА И ПЕРЕТАСКИВАНИЕ ГЛАВНОГО ОКНА
-- ==========================================
...
-- ==========================================
-- ИСПРАВЛЕННЫЙ СКРИПТ ДРАГА И ПЕРЕКЛЮЧЕНИЯ ЭКРАНОВ
-- ==========================================
local function MakeDraggable(dragArea, frameToMove, isIsland)
    local dragging, dragStart, startPos
    local dragDeltaSum = 0
    ...
    dragArea.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            dragDeltaSum = 0 -- Обнуляем счетчик сдвига
        end
    end)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            dragDeltaSum = dragDeltaSum + delta.Magnitude
            
            -- Вычисляем новую позицию в пикселях с учетом размера экрана
            local viewportSize = workspace.CurrentCamera.ViewportSize
            local frameSize = frameToMove.AbsoluteSize
            
            -- Переводим Скейл в Пиксели для правильного математического расчета краев экрана
            local startX_px = startPos.X.Scale * viewportSize.X + startPos.X.Offset
            local startY_px = startPos.Y.Scale * viewportSize.Y + startPos.Y.Offset
            
            local newX = math.clamp(startX_px + delta.X, 0, viewportSize.X - frameSize.X)
            local newY = math.clamp(startY_px + delta.Y, 0, viewportSize.Y - frameSize.Y)
            
            frameToMove.Position = UDim2.new(0, newX, 0, newY)
        end
    end)
    ...
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Если это Островок и сдвиг составил меньше 8 пикселей, считаем это обычным КЛИКОМ
            if isIsland and dragDeltaSum < 8 then
                IslandFrame.Visible = false
                
                local viewportSize = workspace.CurrentCamera.ViewportSize
                local mainFrameSize = MainFrame.AbsoluteSize
                
                -- Центрируем открывающееся окно относительно положения островка
                local targetX = math.clamp(IslandFrame.AbsolutePosition.X - (mainFrameSize.X / 2) + (IslandFrame.AbsoluteSize.X / 2), 10, viewportSize.X - mainFrameSize.X - 10)
                local targetY = math.clamp(IslandFrame.AbsolutePosition.Y - (mainFrameSize.Y / 2) + (IslandFrame.AbsoluteSize.Y / 2), 10, viewportSize.Y - mainFrameSize.Y - 10)
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    local viewportSize = workspace.CurrentCamera.ViewportSize
    local islandSize = IslandFrame.AbsoluteSize
    
    -- Вычисляем координаты островка так, чтобы он сворачивался точно в центр закрытого окна
    local targetX = math.clamp(MainFrame.AbsolutePosition.X + (MainFrame.AbsoluteSize.X / 2) - (islandSize.X / 2), 10, viewportSize.X - islandSize.X - 10)
    local targetY = math.clamp(MainFrame.AbsolutePosition.Y + (MainFrame.AbsoluteSize.Y / 2) - (islandSize.Y / 2), 10, viewportSize.Y - islandSize.Y - 10)
    
    IslandFrame.Position = UDim2.new(0, targetX, 0, targetY)
...
Pages = Instance.new("Frame", MainFrame)
Pages.Name = "Pages"
Pages.BackgroundTransparency = 1
Pages.Position = UDim2.new(0, 150, 0, 45)
Pages.Size = UDim2.new(1, -160, 1, -55)

MainPage = Instance.new("ScrollingFrame", Pages)
...
InfoPage = Instance.new("ScrollingFrame", Pages)
...
-- Создание вкладок на панели навигации
...
TabMainBtn.BackgroundColor3 = CurrentTheme.Accent
TabMainBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
TabMainBtn:SetAttribute("IsActiveTab", true)

-- ==========================================
-- ЛОГИКА ОПРЕДЕЛЕНИЯ И ОТПРАВКИ ЧАТА
-- ==========================================
...
-- Сетевое вращение через BodyAngularVelocity
task.spawn(function()
...
-- Основной цикл обработки физики персонажа
local stepConn = RunService.Stepped:Connect(function()
    pcall(function()
        ...
        -- Rage Quit (Токсичное преследование в лицо)
        ...
        -- 18+ Крепления со скоростной сетевой синхронизацией
...
-- Финальный перевод и рендер элементов
TranslateAll()
