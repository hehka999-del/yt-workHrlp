...
-- Уничтожение старого GUI для предотвращения дублирования в Solara/Xero
...
local function MakeDraggable(dragArea, frameToMove, isIsland)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            ...
            -- Вычисляем новые координаты в пикселях с ограничением по границам экрана
            local viewportSize = workspace.CurrentCamera.ViewportSize
            local frameSize = frameToMove.AbsoluteSize
            
            local newX = math.clamp(startPos.X.Offset + delta.X, 0, viewportSize.X - frameSize.X)
            local newY = math.clamp(startPos.Y.Offset + delta.Y, 0, viewportSize.Y - frameSize.Y)
            
            frameToMove.Position = UDim2.new(startPos.X.Scale, newX, startPos.Y.Scale, newY)
        end
    end)
    ...
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            if isIsland and totalDragDistance < 5 then
                ...
                -- Бесшовная синхронизация: открываем окно точно там, где находился островок (Island)
                local viewportSize = workspace.CurrentCamera.ViewportSize
                local mainFrameSize = MainFrame.AbsoluteSize
                
                -- Смещение рассчитывается так, чтобы центр раскрытого окна совпадал с центром островка
...
MinimizeBtn.MouseButton1Click:Connect(function()
    ...
    -- Смещение рассчитывается так, чтобы центр островка совпадал с центром закрытого окна
...
-- Сетевое вращение через BodyAngularVelocity (100% FE репликация)
task.spawn(function()
        ...
        if TrollState.AttachTarget and TrollState.AttachMode ~= "" and hrp and hum then
            ...
            if targetChar then
                ...
                if thrp then
                    ...
                    -- Маскируем пинг, передавая серверу точную скорость мишени
                    hrp.AssemblyLinearVelocity = thrp.AssemblyLinearVelocity
                    hrp.AssemblyAngularVelocity = thrp.AssemblyAngularVelocity
